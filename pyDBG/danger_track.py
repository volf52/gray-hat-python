...
def single_step_handler(dbg):
    ...
    if crash_encountered:
        ...
        if instruction_count == MAX_INSTRUCTIONS:
            ...
            instruction = dbg.disarm(dbg.context.Eip)
...
pid = int(raw_input("Enter the PID you wish to moniter: "))
dbg.attach(pid)

for func in dangerous_functions.keys():
    func_addr = dbg.func_resolve(dangerous_functions[func], func)
    print "[*] Resolved breakpoint: %s -> 0x%08x" % (func, func_addr)
    dbg.bp_set(func_addr, handler = danger_handler)
    dangerous_functions_resolved[func_addr] = func
